...
namespace FlightReservation
{
    public partial class MainPage : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                Flight sefer1 = new Flight("ABC123-20230601-0330", "BURSA", "İSTANBUL", "2023-06-01", "03:30", "1500");
                Flight sefer2 = new Flight("DEF456-20230601-0700", "BURSA", "İSTANBUL", "2023-06-01", "07:00", "1500");
                Flight sefer3 = new Flight("GHI789-20230601-0900", "BURSA", "İSTANBUL", "2023-06-01", "09:00", "1500");

                seferler.Add((Flight)sefer1);
                seferler.Add((Flight)sefer2);
                seferler.Add((Flight)sefer3);
        ...
        protected void Page_PreRender(object sender, EventArgs e)
        {
            ...
            if (this.IsPostBack && secilen_koltuklar.Count > 0)
            {
                ...
                {
                    pnl_ucak_1.Controls.Clear();
                    fill_accord(0);
                    fill_koltuk_secimi(0);
                }
                else if (state_view_koltuk_array[0] == "1")
                {
                    pnl_ucak_2.Controls.Clear();
                    fill_accord(1);
                    fill_koltuk_secimi(1);
                }
                else
                {
                    pnl_ucak_3.Controls.Clear();
                    fill_accord(2);
        ...
        protected void btn_sefer_sorgula_Click(object sender, EventArgs e)
        {
            ...
            {
                if (txtBox_ArrivedDate.Text != "2023-06-01")
                {
        ...
        {
            Flight current_sefer = (Flight)seferler[sefer_index];

            OleDbCommand cmd_airplane_capacity = new OleDbCommand("select AirplaneCapacity from Booking where BookingID =  '" + current_sefer.BookingId.ToString() + "'", conn);
            conn.Open();
            OleDbDataReader dr_airplane_capacity = cmd_airplane_capacity.ExecuteReader();
            dr_airplane_capacity.Read();
            String sayi = dr_airplane_capacity["AirplaneCapacity"].ToString();
            int airplane_capacity = Int32.Parse(sayi);
            current_sefer.set_Seat_0(airplane_capacity);
            conn.Close();
            ...
            int bus_row = airplane_capacity / 8;

            if (bus_row == 2)
            {
                if (sefer_index == 0)
                {
                    build_otobus_panel(pnl_ucak_1, current_sefer.Seat, sefer_index.ToString());
                }
                else if (sefer_index == 1)
                {
                    build_otobus_panel(pnl_ucak_2, current_sefer.Seat, sefer_index.ToString());
                }
                else
                {
                    build_otobus_panel(pnl_ucak_3, current_sefer.Seat, sefer_index.ToString());
                }
            }
        }
