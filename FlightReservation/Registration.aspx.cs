...
namespace FlightReservation
{
    public partial class Registration : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            Flight current_sefer = (Flight)seferler[Int32.Parse(secilen_sefer[0])];
        ...
        private void Btn_register_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                OleDbCommand bus_booking = new OleDbCommand("select count(*) from Booking where BookingID = '" + ((Flight)seferler[Int32.Parse(secilen_koltuk[0])]).BookingId + "' and PassenderSeatID = '" + secilen_koltuk[1] + "'", conn);
                conn.Open();
                ...
                else
                {
                    if (btn.Enabled)
                    {
                        try
                        {                           
                            OleDbCommand insert_booking = new OleDbCommand("INSERT INTO Booking (BookingID,AirplaneCapacity,PassengerID,PassenderSeatID,PassengerName,PassengerSurname,PassengerGender,PassengerEmail,PassengerPhone) " +
                                "VALUES (@bookingID,@airplaneCapacity,@passengerID,@passenderSeatID,@passengerName,@passengerSurname,@passengerGender,@passengerEmail,@passengerPhone) ", conn);
                            conn.Open();
                            insert_booking.Parameters.AddWithValue("@bookingID", ((Flight)seferler[Int32.Parse(secilen_koltuk[0])]).BookingId);
                            insert_booking.Parameters.AddWithValue("@busCapacity", ((Flight)seferler[Int32.Parse(secilen_koltuk[0])]).Seat.Length.ToString());
